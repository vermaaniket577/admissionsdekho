...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('blog.show', $post->slug) }}" target="_blank" class="text-xs font-bold text-blue-600 hover:underline flex items-center gap-1">
    ...
    <form id="blogForm" action="{{ route('admin.blogs.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
        ...
        <!-- Custom Slug -->
        <div>
            <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">URL Slug</label>
        ...
        <div>
            ...
            <textarea name="excerpt" rows="3"
                      class="w-full bg-slate-50 border border-slate-300 focus:border-blue-500 focus:bg-white text-slate-900 text-sm rounded-xl px-4 py-3 outline-none transition">{{ old('excerpt', $post->excerpt) }}</textarea>
        </div>

        <!-- Featured Image -->
        <div class="grid grid-cols-1 sm:grid-cols-12 gap-6 bg-slate-50 p-5 rounded-2xl border border-slate-200 items-center">
            @if($post->featured_image)
                <div class="sm:col-span-3">
                    <label class="block text-xs font-bold text-slate-700 mb-2">Current Image</label>
                    <div class="w-full h-24 rounded-xl overflow-hidden border border-slate-200 bg-white">
                        <img src="{{ \Illuminate\Support\Str::startsWith($post->featured_image, 'data:image') ? $post->featured_image : asset($post->featured_image) }}" alt="Preview" class="w-full h-full object-cover">
                    </div>
                </div>
            @endif

            <div class="{{ $post->featured_image ? 'sm:col-span-9' : 'sm:col-span-12' }} grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">Replace Uploaded Image</label>
                    <input type="file" name="featured_image" accept="image/*"
                           class="w-full text-xs text-slate-600 file:mr-4 file:py-2 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700">
                </div>
                <div>
                    <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">OR Image Asset URL</label>
                    <input type="text" name="featured_image_url" value="{{ old('featured_image_url', $post->featured_image) }}"
        ...
        <div class="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title', $post->seo?->meta_title) }}"
                ...
                <div>
                    ...
                    <textarea name="meta_description" rows="2"
                              class="w-full bg-white border border-slate-200 text-slate-900 text-xs rounded-xl px-4 py-2.5 outline-none focus:border-blue-500">{{ old('meta_description', $post->seo?->meta_description) }}</textarea>
                </div>
            </div>
        </div>
